...
public class AllMusicPlayer extends InputStream {
    ...
    public long skip(long n) throws IOException {
        ...
        while (remaining > 0) {
            ...
            int r = content.read(buf, 0, toRead);
            if (r < 0) {
                break;
            }
            skipped += r;
    ...
    public void setLocal(long local) throws IOException {
        // 不重建连接（重建会导致 Range 重定位连接被截断），
        // 直接从当前流跳过，保持连接稳定
        if (this.local < local && content != null) {
            long diff = local - this.local;
            byte[] buf = new byte[4096];
            long remaining = diff;
            while (remaining > 0 && content != null) {
                int toRead = (int) Math.min(remaining, buf.length);
                int r = content.read(buf, 0, toRead);
                if (r < 0) break;
                this.local += r;
